...
const RecentMessages = () => {
    ...
    const fetchRecentMessages= async () => {
       try {
           ...
           if(data.success){
            // group messages by sender and get the latest message for eacvh sender
            const groupedMessages = data.messages.reduce((acc, message)=>{
              const senderId= message.from_user_id._id;
              if(!acc[senderId] || new Date(message.createdAt) > new Date(
                acc[senderId].createdAt)){
  ...
  return (
    <div className='bg-white max-w-xs mt-4 p-4 min-h-20 rounded-md shadow-md shadow text-xs text-slate-800'>
